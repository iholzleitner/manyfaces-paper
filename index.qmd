...
Faces are rich sources of information and play a key role in human social perception, strongly drawing attention, informing social inferences, and serving as a primary means of person recognition (e.g., Zebrowitz, 2010; <!--add refs, including recognition sources-->). Various subfields within psychology—including vision science, person perception, social cognition, affective science, and person recognition<!--others?-->—address research questions involving faces and necessitating the use of face images as stimuli. Here, we introduce a novel face image database for research use. This database addresses various limitations of extant available databases and crucially takes a transparent approach, to align with Open Science best practice.

### Existing face databases

A wealth of face stimulus databases exist, created for varying purposes. Many of these are available for research use (see e.g., Workman & Chatterjee 2021 face image meta-database to search many of these<!--cite-->). Researchers therefore have access to a broad variety of existing databases, well-suited to address questions in each subfield. For example, there exist many different databases in which photographed individuals (targets or models) display difference facial expressions of emotion (of use to affective scientists), appear in multiple photos varying in their lighting or angle (useful to person recognition researchers), or belong to various age or racial/ethnic groups (helpful to person perception researchers). However, many studies use the same stimulus databases, which introduces possible problems. For example, 4107 <!--update number before submission-->published papers cite the Karolinska Directed Emotional Faces (Lundquist et al., 1998<!--add ref-->), 2461<!--update number--> cite the Chicago Face Database (Ma et al. <!--add ref-->), and 675 <!--update number-->the Amsterdam Dynamic Facial Expression Set (van der Schalk et al., 2011 <!--add ref-->)<!--other examples to highlight?-->. This frequent use of the same sets of stimuli can cause issues in terms of generalizability if various research questions are tested on only a limited sample of face images. Participants, especially those participating online, may also become familiar with frequently-used stimuli, potentially introducing biases to their responses. 

Of course, not all research involving face images uses stimuli from published databases. Other studies use stimuli collected by the researchers themselves but which they may not be able to share with the research community (e.g., due to ethical limitations). Although this circumvents the issues noted above for frequently-used databases, this introduces its own problems. Chiefly, face images collected by researchers for their own research use are often not well-documented. Although the resulting images may be described and/or illustrated with an example, the process of image collection is rarely described in detail. This is often the case for even those databases that are available for other researchers to use. This lack of documentation limits transparency and can create problems for replication attempts (for example, another researcher attempting to replicate a finding using new face images collected themselves, rather than the images collected by the original researchers; see Třebický et al. 2024 Grim Image preprint <!--add ref-->). 

There are also limitations in terms of many databases’ (both those publicly available and those not) diversity of stimuli. For example, many include images of only White or Western targets/models (e.g., see Cook & Over 2021 <!--add ref-->). This reinforces the centering of Whitness and Western culture in psychological research, importantly limiting the ecological validity and generalizability of conclusions. Recent databases have sought to address this, specifically collecting images of multiracial and non-Western samples, for example (e.g., Chen et al., 2020; Meyers et al., 2024; Trzewik et al., 2025<!--add refs-->), but more such work is needed to further diversify the pool of available face research stimuli. Many world regions remain underrepresented <!--note examples?-->. This may be due, in part, to resource limitations: Equipment and setups for collecting images suitable for research use can be expensive and complex, making database collection not feasible for everyone and everywhere, thus limiting database diversity. Furthermore, to date, no extant database contains images of individuals from *multiple* world regions<!--verify-->.

A final limitation to extant face image databases is their largely static nature. That is, once collected, the number of stimuli available does not change. There are some exceptions to this, with multiple waves of additions to the Chicago Face Database (Ma et al<!--add ref-->), for example. Continued additions to databases are not the norm, however. This not only constrains the number of stimuli in any given database, increasing the likelihood of the frequent reuse of any individual stimulus (with the associated issues of this discussed above), but can also render some databases obsolete after some time (e.g., due to very dated model appearance/styling).

### Open Science considerations

Open and transparent methods are essential for fully understanding, evaluating, and replicating research. Although methods reporting has become more open and accessible with researchers sharing materials such as questionnaire wording, stimuli, and programming scripts, the reporting of face image database collection procedures often lacks crucial details for full transparency and reproducibility—even among recent databases. This matters, because differences in details such as focal length can importantly affect faces' appearance, subsequently impacting perceptions of them (Třebický et al. 2016; see Třebický et al. 2024 Grim Image preprint for further discussion <!--add refs-->).

Big Team Science endeavors and large-scale collaborations are another vital part of improving science. Within psychology, multiple such initiatives have importantly contributed to addressing a variety of research questions, improving diversity and generalizability in the field (e.g., in the domain of face perception, Psychological Science Accelerator project Jones et al. 2021 <!--add ref-->). Such an approach has yet to be applied to stimulus database collection, but represents an opportunity to address many of the limitations of face image databases.
...
To address issues of transparency and reproducibility, we developed a reproducible and openly-available protocol for face image collection. The protocol covers collecting multiple images (varying in standardization, viewing angle, and facial expression) of each target/model to benefit a wide array of possible research areas and questions related to face perception and recognition. We then collected images following this protocol in 20 different labs across the world, spanning 11 countries and five world regions (Europe, Latin America, North America, the Middle East, and Southeast Asia). <!--compare to existing databases?--> This resulted in a diverse set of models and images, more so than would be possible to collect in just one lab. This database is moreover not static, but can be added to in the future by any interested researcher following the protocol. We next validated the database by recruiting online perceivers to rate a subset of the images (i.e., front-facing images) to generate norming data for key social trait perceptions and emotion recognition. We make the image database and norming available for future research
...
To set face perception and recognition Big Team Science in motion and enable conducting multi-site studies, a set of stimuli suitable for most areas (e.g., in terms of research question, geographic location) is needed. Therefore, we developed a protocol for collecting a database of face images. We developed the protocol with maximum usability in mind, not only making it transparent and readable to non-experts, but also creating it for use with attainable (vs. highly specialized and expensive) equipment, with minimal setup requirements, and without expert photography knowledge. Note that the protocol is for in-lab image collection to minimize sources of noise, to enable the collection of models' self-report data, and to ensure the consensual use of models' images (vs. scraping images from online). <!--maybe worth adding a section to the intro about the value of lab-based images vs ambient images &/or artificial faces? other recent database paper have discussed this, could also point to them-->
...
The database itself provides a uniquely diverse face image stimulus set in terms of model nationality and image variety (appearance standardization, viewing angle, and facial expression). It accomplishes this while being transparent and reproducible via the openly-available protocol. Additionally, the database will not remain static: Other researchers can follow the protocol and add to the database, further increasing its sample size and diversity.

The data associated with the images are also valuable. First, we collected models' self-reported demographic information, which is useful for various kinds of research questions. The validation/norming data we collected also provide a useful starting point for researchers interested in using the images. <!-- include? Rating data provide information about the minimum number of raters needed for reliable mean ratings for different judgments (tie to Hehman et al 2025)-->

### Reflections on the Process
Team feedback survey responses [Vít to add] <!--Vít to add-->

The leadership team found that having a broad variety of areas of expertise and diverse research backgrounds within the broader team was invaluable in both developing the database and in troubleshooting issues. We also found that setting out expectations at the start of the project through a collaboration agreement was essential. We also ran into various issues throughout the process, which are common to Big Team Science initiatives. For example, with so many labs and individual researchers involved, timelines for completion necessarily stretched. This made momentum difficult to maintain at times. Planning in generous buffer time for such large-scale projects and managing team members' expectations around timelines is therefore essential. We also faced difficulty in finding an optimal way to communicate effectively with all team members, given varying preferences (e.g., via email vs. other team communication platforms). It is perhaps worth surveying members of a big team at project start about communication possibilities, as well as outlining more specific communication expectations in a collaboration agreement.

Certain methodological choices in the current work ...<!--Lisa to add-->
* Explaining certain methods choices & what others might want to do in the future 
* e.g., in image processing, alignment; image size and face location driven by cropping needs - fix in future 


### Limitations
The emotion validation data suggest that either models struggled to pose the facial expressions of emotion following our simple elicitation instructions, or that perceivers have difficulty in identifying emotions from posed facial expressions. In support of the former idea, models did self-report difficulty with the facial expression posing. In support of the latter idea, recent research shows emotion perception to be more accurate for spontaneous than posed expressions (<!--Amy Dawel's recent work, if i remember correctly?-->). <!--cite discussions within emotion literature about value of posed expressions, or best ways to elicit expressions-->This is a consideration for future research and possible updates to the protocol. 

Other protocol limitations & future improvements? [Vít to add] <!--Vít to add-->

We collected images of 211 models, but no single image type included images of all models. Rather, the maximum number of models represented in a single image type was 205 images (standardized neutral, front-facing). This led to missing perceived age/ethnicity data for size models who had unstandardized but not standardized neutral images available.
