...
Next, we calculated intraclass correlation coefficients, Cronbach's α, and McDonald's ωt for emotion intensity ratings. The number of raters ranged from `r emo_icc_table |> pull(n_raters) |> min()` to `r emo_icc_table |> pull(n_raters) |> max()`. The average reliability across raters was excellent for all emotions, with ICC(2,k) ranging from `r min(emo_icc_table[["ICC(2,k)"]])` to `r max(emo_icc_table[["ICC(2,k)"]])`, Cronbach’s α ranging from `r min(emo_alpha_omega_table$alpha)` to `r max(emo_alpha_omega_table$alpha)`, and  McDonald's ωt ranging from `r min(emo_alpha_omega_table$omega_t)` to `r max(emo_alpha_omega_table$omega_t)` (see @tbl-emo-icc and @tbl-emo-alpha-omega).
